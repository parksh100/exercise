<template>
  <div class="container">
    ...
    <table class="table table-bordered" style="table-layout: fixed">
      <tbody class="table-group-divider">
        <tr>
          <th class="bg-light" style="width: 30%">세금계산서발행액(VAT포함)</th>
          <th class="bg-light" style="width: 30%">입금액(VAT포함/원)</th>
          <th class="bg-light" style="width: 20%">공제액</th>
          <th class="bg-light" style="width: 20%">메모</th>
        </tr>
        <tr class="text-center">
          <td>
            {{
              $convertNumberFormat(
                dBTaxInvoiceInfo.tax_invoice_amount * 1.1,
                '#,###'
              )
            }}원
          </td>
          ...
          <td>
            {{ $convertNumberFormat(this.deductAmount, '#,###') }}
          </td>
          <td>
            <textarea
              class="form-control"
    ...
    <div class="text-end">
      ...
      <button class="btn btn-primary" @click="doSaveCash">저장</button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  async mounted() {
    ...
    this.getList()
    this.getTaxInvoiceInfo()
    this.getDeductAmount(this.id)
  },
  unmounted() {},

  methods: {
    // 등록된 심사정보 Join & 조회 적용
    async getTaxInvoiceInfo() {
      const loader = this.$loading.show({ canCancel: false })
      // console.log(this.user.userInfo.email)
      // const searchName = `%${this.searchName.toLowerCase()}%`
      // console.log(searchName)

      const result = await this.$get(`/api/tax/${this.id}`)
      // console.log('result', result)
      this.dBTaxInvoiceInfo = result
      console.log('dBTaxInvoiceInfo', this.dBTaxInvoiceInfo)
    ...
    // 공제액 산출식
    async getDeductAmount(id) {
      const loader = this.$loading.show({ canCancel: false })
      const result = await this.$get(`/api/customer/cert/list/detail/${id}`)
      console.log('deDuctResult', result)
      console.log('audit_type', result.audit_type)
      console.log('audit_type', result.audit_standard)
      console.log('audit_type', JSON.stringify(result.audit_standard))
      console.log('length', JSON.parse(result.audit_standard).length)
      const auditStandardArray = JSON.parse(result.audit_standard)

      if (result.audit_type === '최초') {
        if (auditStandardArray.length === 1) {
          if (
            auditStandardArray.includes('QMS') ||
            auditStandardArray.includes('CGMP')
          ) {
            this.deductAmount = 225000
          } else if (
            auditStandardArray.includes('EMS') ||
            auditStandardArray.includes('OHSMS')
          ) {
            this.deductAmount = 375000
          }
        }
        if (auditStandardArray.length === 2) {
          if (
            auditStandardArray.includes('QMS') ||
            auditStandardArray.includes('CGMP')
          ) {
            this.deductAmount = 450000
          } else if (auditStandardArray.includes('QMS') === false) {
            this.deductAmount = 600000
          }
        }
        if (auditStandardArray.length === 3) {
          if (
            auditStandardArray.includes('QMS') ||
            auditStandardArray.includes('CGMP')
          ) {
      ...
      if (result.audit_type === '사후' || result.audit_type === '전환사후') {
        if (auditStandardArray.length === 1) {
          if (
            auditStandardArray.includes('QMS') ||
            auditStandardArray.includes('CGMP')
      ...
      if (result.audit_type === '갱신' || result.audit_type === '전환갱신') {
        if (auditStandardArray.length === 1) {
          if (
            auditStandardArray.includes('QMS') ||
            auditStandardArray.includes('CGMP')
    ...
    doSaveCash() {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          const r = await this.$post('/api/fee', {
            param: {
