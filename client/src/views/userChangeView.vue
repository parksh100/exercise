<template>
  <div class="container mt-5">
    <h3 class="mb-4 fw-bold text-center">심사원정보등록 수정</h3>
    <hr />
    <div class="row mb-3">
...
export default {
  ...
  created() {
    ...
    console.log('userList에서 넘어온 id:', this.id)
  ...
  methods: {
    async getUserById() {
      ...
      const result = await this.$get(`/api/user/${this.id}`)
      console.log(result)
      console.log(result.user_email)

      this.auditor.email = result.user_email
      this.auditor.name = result.user_name
      this.auditor.role = result.user_role
      this.auditor.pw = result.user_pw
      this.auditor.iaf_code = JSON.parse(result.user_auditable_code)
      loader.hide()
    },
    ...
    async doSave() {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          const r = await this.$put(`/api/user/${this.id}`, {
            param: {
