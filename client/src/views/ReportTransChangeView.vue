          ...
          <td colspan="5">
            <div class="form-check form-check-inline">
              <input
                ...
                value="정상유지"
                v-model="addTransReport.certification_status"
                name=""
              />
          ...
          <td colspan="5">
            <div class="form-check form-check-inline">
              <input
                ...
                value="불만"
                name=""
                v-model="addTransReport.change_reason"
              />
            ...
            <div class="form-check form-check-inline">
              <input
                ...
                v-model="addTransReport.change_reason"
              />
              <label class="form-check-label" for="change_reason_3">기타</label>
            </div>
          </td>
          <!-- <th class="bg-light">대표자</th>
          <td>{{ list.name_ko }}</td>
          ...
          <td colspan="5">
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="recent_audit_result_1"
                value="최초심사"
                v-model.trim="addTransReport.recent_audit"
                name=""
              />
              <label class="form-check-label" for="recent_audit_result_1"
                >최초심사</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="recent_audit_result_2"
                value="사후심사"
                v-model.trim="addTransReport.recent_audit"
                name=""
              />
              <label class="form-check-label" for="recent_audit_result_2"
                >사후심사</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="recent_audit_result_3"
                value="갱신심사"
                v-model.trim="addTransReport.recent_audit"
                name=""
              />
              <label class="form-check-label" for="recent_audit_result_3"
                >갱신심사</label
              >
            </div>
          </td>
          ...
          <td colspan="5">
            <div class="form-check form-check-inline">
              <input
                ...
                value="접수실적있음/조치완료"
                v-model.trim="addTransReport.complaint_handling"
                name=""
              />
              <label class="form-check-label" for="complaint_handling_1"
                >접수실적있음/조치완료</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                ...
                name=""
              />
              <label class="form-check-label" for="complaint_handling_2"
                >접수실적있음/조치미완료</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="complaint_handling_3"
                value="접수실적없음"
                v-model.trim="addTransReport.complaint_handling"
                name=""
              />
              <label class="form-check-label" for="complaint_handling_3"
                >접수실적없음</label
              >
            </div>
          </td>
          ...
          <td colspan="5">
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="certification_validity_1"
                value="기존 인증서의 효력이 유지되고 있으며 인증전환에 하자
                없음(인증추천함)"
                v-model.trim="addTransReport.certification_validity"
                name=""
              />
              <label class="form-check-label" for="certification_validity_1"
                >기존 인증서의 효력이 유지되고 있으며 인증전환에 하자
                없음(인증추천함)</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="certification_validity_2"
                value="일부 미비사항이 발견되었으며 이에 대한 조치가 선결되어야
                함"
                v-model.trim="addTransReport.certification_validity"
                name=""
              />
              <label class="form-check-label" for="certification_validity_2"
                >일부 미비사항이 발견되었으며 이에 대한 조치가 선결되어야
                함</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="certification_validity_3"
                value="인증전황이 불가능하며 확인심사 또는 신규심사로 처리되어야
                함"
                v-model.trim="addTransReport.certification_validity"
              />
              <label class="form-check-label" for="certification_validity_3"
                >인증전황이 불가능하며 확인심사 또는 신규심사로 처리되어야
                함</label
              >
            </div>
            <div class="form-check form-check-inline">
              <input
                class="form-check-input"
                type="radio"
                id="certification_validity_4"
                value="객관적 증거부족으로 추후 재방문 심사를 통하여 처리함"
                v-model.trim="addTransReport.certification_validity"
              />
              <label class="form-check-label" for="certification_validity_4"
          ...
          <td colspan="5">
            <div>
              <textarea
                class="form-control"
                maxlength="100"
                v-model.trim="addTransReport.total_evaluation"
              ></textarea
              >({{ addTransReport.total_evaluation.length }}/100)
            </div>
          </td>
        </tr>
        <tr>
          ...
          <td colspan="5">
            <div>
              <input
                ...
                name=""
                id=""
                v-model="addTransReport.trans_audit_date"
              />
            </div>
          </td>
        </tr>
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-9">
                ...
                <input
                  ...
                  v-model="addTransReport.attachment_transCert_originalname"
                  v-if="addTransReport.attachment_transCert_originalname"
                />
              </div>
              <!-- <img
        :src="imgSrc"
...
export default {
  ...
  methods: {
    ...
    async getTransReportInfo() {
      ...
      console.log(result)
      this.addTransReport.certification_status = result.certification_status
      this.addTransReport.change_reason = result.change_reason
      this.addTransReport.recent_audit = result.recent_audit
      this.addTransReport.complaint_handling = result.complaint_handling
      this.addTransReport.certification_validity = result.certification_validity
      this.addTransReport.total_evaluation = result.total_evaluation
      this.addTransReport.attachment_transCert_originalname =
        result.attachment_transCert_originalname
      this.addTransReport.attachment_transCert = result.attachment_transCert
      this.addTransReport.attachment_transCert_ext =
        result.attachment_transCert_ext
      this.addTransReport.trans_audit_date = Intl.DateTimeFormat(
        'fr-CA'
      ).format(new Date(result.trans_audit_date))
      loader.hide()
    },
    async getList() {
      const loader = this.$loading.show({ canCancel: false })
    ...
    async doSave() {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          const r = await this.$put(`/api/report/trans/${this.id}`, {
            param: {
