  ...
  <div class="container">
    <div class="col mb-3">
      <div class="text-center">
        <h1 class="fw-bold">S1보고서 업로드 수정</h1>
      </div>
      <hr />
    </div>
        ...
        <tr>
          ...
          <td style="width: 75%">
            ...
            <input
              ...
              v-model="dbUploadedS1Info.report_s1_originalname"
              v-if="dbUploadedS1Info.report_s1_originalname"
            />
          </td>
        </tr>
        <tr>
          <td>
            ...
            <div class="text-end">
              ...
              <button class="btn btn-primary" @click="doSaveS1">저장</button>
            </div>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  async mounted() {
    ...
    this.getList()
    this.getDbUploadS1ReportList()
  },
  unmounted() {},

  methods: {
    async getDbUploadS1ReportList() {
      const loader = this.$loading.show({ canCancel: false })
      const result = await this.$get(`/api/upload/s1/${this.id}`)
      console.log('업로드s1보고서', result)
      this.dbUploadedS1Info = result
      loader.hide()
    },
    ...
    doSaveS1() {
      if (this.file_s1Report_originalname === '') {
        this.$swal('S1보고서를 첨부하세요.')
        return
      }

      this.$swal({
        title: 'S1보고서를 수정 하시겠습니까?',
        // text: "You won't be able to revert this!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
          ...
          const r = await this.$put(`/api/upload/s1/${this.id}`, {
            param: {
              report_s1: this.file_s1Report,
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          const r = await this.$post('/api/upload/s2', {
            param: {
              report_s2: this.file_s2Report,
              report_s2_originalname: this.file_s2Report_originalname,
              report_ext: this.file_s2Report_Ext,
              audit_no: this.list.audit_no,
              business_no: this.list.business_no
            }
          })
