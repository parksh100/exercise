  ...
  <div class="container">
    <div class="col mb-3">
      <div class="text-center">
        <h1 class="fw-bold">S2보고서 업로드 수정</h1>
      </div>
      <hr />
    </div>
        ...
        <tr>
          ...
          <td style="width: 75%">
            ...
            <input
              ...
              v-model="dbUploadedS2Info.report_s2_originalname"
              v-if="dbUploadedS2Info.report_s2_originalname"
            />
          </td>
        </tr>
        <tr>
          <td>
            ...
            <div class="text-end">
              ...
              <button class="btn btn-primary" @click="doSaveS2">저장</button>
            </div>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  async mounted() {
    ...
    this.getList()
    this.getDbUploadS2ReportList()
  },
  unmounted() {},

  methods: {
    async getDbUploadS2ReportList() {
      const loader = this.$loading.show({ canCancel: false })
      const result = await this.$get(`/api/upload/s2/${this.id}`)
      console.log('업로드s2보고서', result)
      this.dbUploadedS2Info = result
      loader.hide()
    },
    ...
    doSaveS2() {
      if (this.file_s2Report_originalname === '') {
        this.$swal('S2보고서를 첨부하세요.')
        return
      }

      this.$swal({
        title: 'S2보고서를 수정업로드 하시겠습니까?',
        // text: "You won't be able to revert this!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
          ...
          const r = await this.$put(`/api/upload/s2/${this.id}`, {
            param: {
