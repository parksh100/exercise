<template>
  <div class="container">
            ...
            <td style="width: 30%">
              ...
              <div style="float: left; width: 50%">
                <button
                  ...
                  @click="giveNumber"
                  :disabled="cr.audit_no ? true : false"
                >
                  심사번호부여
                </button>
              </div>
    ...
    <div class="text-end">
      <button class="btn btn-primary me-1" @click="doSave">저장</button>
      <button class="btn btn-primary me-1" @click="goToList">목록</button>
    </div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    giveNumber() {
      ...
      if (month < 10) {
        this.number.month = '0' + month
      } else {
        this.number.month = month
      }
      ...
      for (let i = 0; i < customerStandard.length; i++) {
        ...
        for (let j = 0; j < standard.length; j++) {
          if (customerStandard[i] === standard[j]) {
            this.number.scheme += standardCode[j]
          }
        }
      }
      console.log('규격코드', this.number.scheme)

      // 일련번호 3자리
      const serialNo = Math.floor(Math.random() * 1000) + 1
      console.log('일련번호', serialNo)
      ...
      console.log('최종번호', this.cr.audit_no)

      // 심사구분(단일, 통합)
      if (customerStandard.length > 1) {
        this.cr.class_audit = '통합심사'
      } else {
        this.cr.class_audit = '단일심사'
      }
    },
    ...
    async doSave() {
      ...
      // }
      console.log(this.cr.audit_no.length)
